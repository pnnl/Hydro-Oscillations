...
EPCL script "2_Modifying_PowerFlow_for_WHV.p" is used to modify the power flow case in step 2. This script sets the new Pgen and Pmax values calculated in step 1, for one hydro generator at a time and runs a governor power flow each time ensuring re-dispatch. The output of this step is a new power flow file (.sav extension) which will be used in the subsequent step.
